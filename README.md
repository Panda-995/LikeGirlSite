...
#### 启动方法
- Docker + SQLite（推荐）
    - 构建镜像：`docker build -t likegirlsite-sqlite .`
    - 运行容器：`docker run -d --name likegirlsite -p 8080:80 -v likegirl-data:/var/www/html/data likegirlsite-sqlite`
    - 或使用 Compose：`docker compose up -d --build`
